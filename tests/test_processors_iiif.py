...
def test_load_image():
    ...
    response = iiif.load_image(data, url="https://example.edu/iiif", api_get_function=_test_api_get)
    assert isinstance(response, RequestsResponse)
    assert response.ok

    # Test for no url passed in
    response = iiif.load_image(data, url=None, api_get_function=_test_api_get)
    assert isinstance(response, RequestsResponse)
    assert response.ok

    # Test for invalid url
    with raises(HTTPException) as http_error:
        response = iiif.load_image(data, url="invalid:/url/format", api_get_function=_test_api_get)
    assert http_error.type.code == 400

    # Test that fail fails correctly
    response = iiif.load_image(data, url="https://example.edu/iiif", api_get_function=_test_api_get_fail)
    assert isinstance(response, type(None))

    # Test that the api service shows correctly as 'unavailable'
    data['on_fail'] = 0
    with raises(HTTPException) as http_error:
        response = iiif.load_image(data, url="https://example.edu/iiif", api_get_function=_test_api_get_unavailable)
