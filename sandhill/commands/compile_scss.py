...
def compile_scss(scss_dir=None, css_dir=None):
    ...
    '''
    try:
        run_compile(scss_dir, css_dir)
    except Exception as exc:
        click.echo(click.style(str(exc), bold=True, fg='red'))
...
def run_compile(scss_dir=None, css_dir=None):
    ...
    and save the results to the css directory.
    args:
        scss_dir (str) [Optional]: Path to search for scss files
            default: instance directory + static/scss
        css_dir (str) [Optional]: Path to output the compiled css
            default: instance directory + static/css
    returns:
        (int): 0 if successful (else raises an exception)
    raises:
        (Exception): any error that occured
    '''
    ...
    if (scss_dir and not isinstance(scss_dir, str)) or (css_dir and not isinstance(css_dir, str)):
        raise Exception("One or both of the provided arguments were not string values")
    ...
    if not os.path.exists(scss_dir):
        raise Exception(f"scss-dir path does not exist. {scss_dir}")
    if not os.path.exists(css_dir):
        raise Exception(f"css-dir path does not exist. {scss_dir}")

    # Try to compile!
    try:
        sass.compile(dirname=(scss_dir, css_dir))
    except sass.CompileError as exc:
        raise Exception(f"Unable to compile scss in dir {scss_dir}: {exc}")
