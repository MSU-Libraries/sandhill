...
def test_conditions():
    ...
    response = fedora3.load_datastream(data_dict, url="https://test.example.edu", api_get_function=_test_api_get)
    ...
    data_dict["view_args"]["action"] = "download"
    with raises(HTTPException) as http_error:
        response = fedora3.load_datastream(data_dict, url="invalid url", api_get_function=_test_api_get)
    assert http_error.type.code == 400

    # Test for invalid url
    with raises(HTTPException) as http_error:
        response = fedora3.load_datastream(data_dict, url=None, api_get_function=_test_api_get)
    assert http_error.type.code == 400

    # Test for failed call to fedora
    with raises(HTTPException) as http_error:
        response = fedora3.load_datastream(data_dict, url="https://test.example.edu", api_get_function=_test_api_get_unavailable)
    assert http_error.type.code == 503

    # Test for non-503 fedora call exception
    with raises(HTTPException) as http_error:
        response = fedora3.load_datastream(data_dict, url="https://test.example.edu", api_get_function=_test_api_get_fail)
