...
def test_select():
    ...
    response = solr.select(data_dict, solr_url="https://test.example.edu", api_get_function=_test_api_get_json)
    assert isinstance(response, dict)

    # Test that a missing Solr URL fails
    with raises(HTTPException) as http_error:
        response = solr.select(data_dict, solr_url=None, api_get_function=_test_api_get_json)
    ...
    # Test with a bad URL
    with raises(HTTPException) as http_error:
        response = solr.select(data_dict, solr_url="not_a_url", api_get_function=_test_api_get_json)
    assert http_error.type.code == 400

    # Test a failed call
    with raises(HTTPException) as http_error:
        response = solr.select(data_dict, solr_url="https://test.example.edu", api_get_function=_test_api_get_fail)
    assert http_error.type.code == 500

    # Test an unavailable call
    with raises(HTTPException) as http_error:
        response = solr.select(data_dict, solr_url="https://test.example.edu", api_get_function=_test_api_get_unavailable)
...
def test_select_record():
    ...
    # Test for successful response
    response = solr.select_record(data_dict, solr_url="https://test.example.edu", api_get_function=_test_api_get_json)
    ...
    with raises(HTTPException) as http_error:
        result = solr.select_record(data_dict=None, solr_url="https://test.example.edu", api_get_function=_test_api_get_json)
    assert http_error.type.code == 400

    with raises(HTTPException) as http_error:
        result = solr.select_record(data_dict={}, solr_url="https://test.example.edu", api_get_function=_test_api_get_json)
    assert http_error.type.code == 400
    ...
    # Test a Solr error
    response = solr.select_record(data_dict, solr_url="https://test.example.edu", api_get_function=_test_api_get_json_error)
    assert response is None

    # Test if a dictionary is not returned
    with raises(HTTPException) as http_error:
        result = solr.select_record(data_dict, solr_url="https://test.example.edu", api_get_function=_test_api_get)
    assert http_error.type.code == 503
...
def test_search():
    ...
    with app.test_request_context('/search'):
        response = solr.search(data_dict, solr_url="https://test.example.edu", api_get_function=_test_api_get_json)
        assert isinstance(response, dict)
        assert response
