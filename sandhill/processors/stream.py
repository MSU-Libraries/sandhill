...
def stream(data_dict):
    ...
    '''
    allowed_headers = [
        'Content-Type', 'Content-Disposition', 'Content-Length', 'Range', 'accept-ranges', 'Content-Range'
    ]
    ...
    if isinstance(resp, RequestsResponse) and not resp:
        ...
        abort(503)
    stream_response = FlaskResponse(resp.iter_content(chunk_size=app.config['STREAM_CHUNK_SIZE']), status=resp.status_code)
